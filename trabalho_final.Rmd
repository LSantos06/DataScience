...
dificuldades = unb.prof.biomed.df$resumo_cv[length(unb.prof.biomed.df$resumo_cv)] %>% tolower %>% 
  removePunctuation %>% removeWords(., c(stopwords("pt"),"engenharia","federal","faculdade","pósgraduação","universidade"))
wordcloud(enc2native(dificuldades), min.freq=2,max.words=100, random.order=F,colors=brewer.pal(8,"Dark2"), use.r.layout=TRUE, rot.per= 0.5)
```
...
docs <- tm_map(docs, removeWords, c("pósgraduação", "federal","elétrica","pósgraduação")) 
docs <- tm_map(docs, removePunctuation)
docs <- tm_map(docs, stripWhitespace)
dtm <- TermDocumentMatrix(docs)
dtm$dimnames$Terms <- enc2native(dtm$dimnames$Terms )
m <- as.matrix(dtm)
m
...
Vê-se que as palavras mais usadas são relacionadas a saúde, como "saúde", "nanotecnologia" e "biomédica". Além de palavras comuns do português.

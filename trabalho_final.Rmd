---
title: Avaliação dos Programas de Pós-graduação Engenharia Biomédica, Engenharia de Sistemas Eletrônicos e de Automação, Engenharia Elétrica P7 e Engenharia Elétrica P8 e dos Currículos dos Professores e Pesquisadores Cadastrados
subtitle: Tópicos Avançados em Computadores CIC/UnB 2018.2 Turma D "Ciência de Dados para Todos (Data Science
  For All)".  Professor Ricardo Barros Sampaio e Professor Jorge Henrique Cabral Fernandes.
author: "Marcos Vinicius Prescendo Tonin 14/0153233, Lucas Nascimento Santos Souza 14/0151010 e Carlos Aragão"
date: "02/12/2018"
output:
  html_document:
    df_print: paged
...
Analisando o resultado dos scripts, pode-se perceber que os professores do programa possuem 4 áreas de atuação, sendo a mais popular ENGENHARIAS, com 57 ocorrênias da mesma nos perfis dos docentes. A principal produção dos docentes são períodicos com 226 produções bibliográficas no total.
...
Analisando as publicações, percebe-se que o programa obteve 206 no total durante sua existência (2010-2017). A informação que se faz interessante é o períodico que mais possui publicações do programa, sendo ele denominado "Revista Brasileira de Engenharia Biomédica (Impresso)", que possui grande relação com o outro programa de pós-graduação analisado (Engenharia Biomédica)
...
Já no quesito de orientações, o programa possui 149 no total, sendo 141 delas com apenas um orientador. A instituição com mais orientações é a Universidade de Brasília e o curso com mais orientações "ENGENHARIA DE SISTEMAS ELETRÔNICOS E DE AUTOMAÇÃO", que é coerente com o programa de pós-graduação. A maioria das orientações foram dissertações de mestrado com 102 no total, e o docente com mais orientações é o professor Francisco Assis de Oliveira Nascimento com 16 orientações. A maioria dos alunos são bolsistas com 93 no total, sendo que a agência financiadora mais presente no programa é a Coordenação de Aperfeiçoamento de Pessoal de Nível Superior com 59 financiamentos.
...
A avaliação será feita de forma gráfica, sendo estes gráficos exibidos a seguir:

#### Engenharia Biomédica

Quantidade de orientações feitas por ano: 
```{r}
unb.adv.biomed.df %>%
group_by(ano) %>%
...
Quantidade de orientações separadas por tipo de publicação:
```{r}
unb.adv.biomed.df %>%
group_by(natureza) %>%
summarise(Quantidade = n()) %>%
...
```{r}
cursos_d5
pie(cursos_d5,clockwise=F,main="Cursos mais presentes")
```

Quantidade de bolsistas e não bolsistas:
```{r}
pie(table(unb.adv.biomed.df$bolsa),main="Bolsas")
```

Quantidade de publicações feitas por ano: 
```{r}
unb.pub.biomed.df %>%
group_by(ano) %>%
summarise(Quantidade = n()) %>%
...
Autores com mais participações em publicações:
```{r}
head(sort(table(toupper(unlist(strsplit(unb.pub.biomed.df$autores,";")))),decreasing = TRUE),n=10)
```
Vemos que há algumas incoeráncia neste dado acima, isto se dá pela falta de uniformidade dos dados.

Perfis de docentes por senioridade:
```{r}
unb.prof.biomed.df %>%
...
Quantidade de orientações concluídas no doutorado por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
Quantidade de orientações concluídas no mestrado por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
Quantidade de orientações concluídas no pós-doutorado por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
Quantidade de orientações concluídas em outros tipos por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
Quantidade de eventos por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
filter(!is.na(EVENTO))%>%  
...
Quantidade de periódicos por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
Quantidade de livros por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
filter(!is.na(LIVRO))%>%  
...
Quantidade de texto em jornais por professor:
```{r}
unb.prof.biomed.df %>%
group_by(nome) %>%
...
```{r}
list <- c(rbind(unb.graph.biomed$links$source,unb.graph.biomed$links$target))
idx <- match(list, unb.graph.biomed$nodes$id)
names <- unb.graph.biomed$nodes$properties$name
graph_names <- names[idx]
g <-graph(graph_names, directed=TRUE)
plot(g, layout = layout_(g, nicely()),
     edge.width = 1,
     edge.arrow.width = 1,
     vertex.size = 5,
     edge.arrow.size = 1,
...
Nuvem de palavras baseadas no resumo do currículo dos professores:
```{r}
...
Nuvem de palavras baseadas no título das orientações:
```{r}
...
Quantidade de bolsistas e não bolsistas:
```{r}
pie(table(unb.adv.auto.df$bolsa),main="Bolsas")
```
...
Relações entre professores:
```{r}
list <- c(rbind(unb.graph.auto$links$source,unb.graph.auto$links$target))
idx <- match(list, unb.graph.auto$nodes$id)
names <- unb.graph.auto$nodes$properties$name
graph_names <- names[idx]
g <-graph(graph_names, directed=TRUE)
plot(g, layout = layout_(g, nicely()),
     edge.width = 1,
...
Nuvem de palavras baseadas no resumo do currículo dos professores:
```{r}
unb.prof.auto.df$resumo_cv <- iconv(unb.prof.auto.df$resumo_cv,from="UTF-8", to = "ASCII//TRANSLIT")
df_wc <- data.frame(doc_id = unb.prof.auto.df$idLattes, text = unb.prof.auto.df$resumo_cv, stringsAsFactors = FALSE )
docs <- Corpus(DataframeSource(df_wc))
docs <- tm_map(docs, content_transformer(tolower))
docs <- tm_map(docs, removeNumbers)
docs <- tm_map(docs, removeWords, stopwords("pt"))
...
Nuvem de palavras baseadas no título das orientações:
```{r}
unb.adv.auto.df$titulo <- iconv(unb.adv.auto.df$titulo,from="UTF-8", to = "ASCII//TRANSLIT")
df_wc <- data.frame(doc_id = unb.adv.auto.df$idLattes1, text = unb.adv.auto.df$titulo, stringsAsFactors = FALSE )
docs <- Corpus(DataframeSource(df_wc))
docs <- tm_map(docs, content_transformer(tolower))
docs <- tm_map(docs, removeNumbers)
docs <- tm_map(docs, removeWords, stopwords("pt"))
...
Quantidade de bolsistas e não bolsistas:
```{r}
pie(table(unb.adv.ele_p8.df$bolsa),main="Bolsas")
```
...
Relações entre professores:
```{r}
list <- c(rbind(unb.graph.ele_p8$links$source,unb.graph.ele_p8$links$target))
idx <- match(list, unb.graph.ele_p8$nodes$id)
names <- unb.graph.ele_p8$nodes$properties$name
graph_names <- names[idx]
g <-graph(graph_names, directed=TRUE)
plot(g, layout = layout_(g, nicely()),
     edge.width = 1,
...
Quantidade de bolsistas e não bolsistas:
```{r}
pie(table(unb.adv.ele_p7.df$bolsa),main="Bolsas")
```
...
Relações entre professores:
```{r}
list <- c(rbind(unb.graph.ele_p7$links$source,unb.graph.ele_p7$links$target))
idx <- match(list, unb.graph.ele_p7$nodes$id)
names <- unb.graph.ele_p7$nodes$properties$name
graph_names <- names[idx]
g <-graph(graph_names, directed=TRUE)
plot(g, layout = layout_(g, nicely()),
     edge.width = 1,
